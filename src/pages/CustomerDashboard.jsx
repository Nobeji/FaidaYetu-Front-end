    ...
    <DashboardShell brand="FaidaYetu" brandSub="Customer Portal" navItems={navItems} profile={
      ...
      <div>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight: 700, color: '#0f172a', margin: 0 }}>Find Poultry</h1>
          <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>Discover fresh products near you</p>
        </div>
          ...
          {editLocation ? (
            <>
              ...
              <div style={{ display: 'flex', gap: 8, padding: 10, borderTop: '1px solid #e2e8f0', background: '#fff' }}>
                <button onClick={() => { setEditLocation(false); setEditCoords({ lat: userLocation[0], lng: userLocation[1] }); }} style={{ flex: 1, padding: '8px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontWeight: 500, fontSize: 12, color: '#475569' }}>Cancel</button>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, color: '#0f172a', margin: 0 }}>Recommended Near You</h3>
          <span onClick={() => navigate('/customer/marketplace')} style={{ fontSize: 12, fontWeight: 600, color: '#0a6e46', cursor: 'pointer' }}>View All →</span>
        </div>
        ...
        {orders.length > 0 && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, marginTop: 32 }}>
              <h3 style={{ fontSize: 15, fontWeight: 600, color: '#0f172a', margin: 0 }}>Recent Orders</h3>
              <span onClick={() => navigate('/customer/orders')} style={{ fontSize: 12, fontWeight: 600, color: '#0a6e46', cursor: 'pointer' }}>View All →</span>
            </div>
              ...
              {orders.slice(0, 5).map((o, i) => (
                <div key={o.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: i < orders.slice(0, 5).length - 1 ? '1px solid #f1f5f9' : 'none', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <button onClick={() => navigate('/customer/orders')} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontWeight: 500, fontSize: 11, color: '#475569' }}>Details</button>
                    ...
                    {pendingPayments[o.id] === 'pending' && (
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                        <span style={{ padding: '6px 12px', borderRadius: 8, background: '#fef9c3', color: '#a16207', fontWeight: 600, fontSize: 11 }}>⌛ Pending</span>
                        <button onClick={() => checkPaymentStatus(o.id)} disabled={checkingPayments[o.id]} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid #fde68a', background: '#fff', color: '#a16207', cursor: 'pointer', fontWeight: 600, fontSize: 11, display: 'flex', alignItems: 'center', gap: 4 }}>
                          {checkingPayments[o.id] && <Spinner size={10} color="#a16207" />}Check
                        </button>
                      </div>
                    )}
                    {o.paid && <span style={{ padding: '6px 12px', borderRadius: 8, background: '#f0fdf4', color: '#15803d', fontWeight: 600, fontSize: 11 }}>✓ Paid</span>}
                    {o.status === 'cancelled' && <span style={{ padding: '6px 12px', borderRadius: 8, background: '#fef2f2', color: '#dc2626', fontWeight: 600, fontSize: 11 }}>✕ Cancelled</span>}
                    {canCancel(o.status) && (
                      <button onClick={() => handleCancelOrder(o.id)} disabled={cancellingOrders[o.id]} style={{ padding: '6px 12px', borderRadius: 8, background: '#fef2f2', color: '#dc2626', border: 'none', cursor: 'pointer', fontWeight: 500, fontSize: 11, display: 'flex', alignItems: 'center', gap: 4 }}>
                        {cancellingOrders[o.id] && <Spinner size={10} color="#dc2626" />}Cancel
                      </button>
                    )}
                    {o.status === 'in_transit' && (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }} onClick={() => !paying && setShowPayModal(null)}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 24, width: '100%', maxWidth: 440, border: '1px solid #e2e8f0' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', margin: 0 }}>Pay for Order #{showPayModal.id}</h3>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => setShowPayModal(null)} disabled={paying} style={{ flex: 1, padding: '12px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', color: '#475569', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
