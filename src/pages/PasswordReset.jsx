    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, color: '#0f172a' }}>{t('auth.forgotPassword')}</h1>
          <p style={{ fontSize: 14, color: '#64748b', marginTop: 4 }}>{t('contact.respondWithin')}</p>
        </div>
        {!sent ? (
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 13, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 8 }}>{t('auth.email')}</label>
            ...
            <button type="submit" style={{ width: '100%', padding: '14px', borderRadius: 12, background: '#0a6e46', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 15, fontWeight: 700 }}>{t('contact.sendMessage')}</button>
          </form>
        ) : (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', marginBottom: 8 }}>{t('contact.sent')}</h2>
            <p style={{ fontSize: 14, color: '#64748b', marginBottom: 20 }}>We've sent a reset link to <strong style={{ color: '#0f172a' }}>{email}</strong></p>
            <button onClick={() => setSent(false)} style={{ padding: '12px 32px', borderRadius: 12, background: '#f1f5f9', border: '1px solid #e2e8f0', color: '#475569', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>{t('contact.sendAnother')}</button>
          </div>
        )}
        <div style={{ textAlign: 'center', marginTop: 20 }}>
          <Link to="/auth" style={{ fontSize: 14, color: '#0a6e46', fontWeight: 600 }}>{t('common.back')}</Link>
        </div>
      </div>
    </div>
  );
