  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 28 }}>
        {[
          { label: 'Forecast (Next Week)', value: data.weeklyForecast, sub: data.model === 'prophet' ? `±${Math.round((data.weeklyForecastUpper - data.weeklyForecastLower) / 2)} range • ${data.weeklyRevenue}` : `${data.weeklyRevenue}`, icon: '📅', gradient: 'linear-gradient(135deg, #111, #333)' },
          { label: 'Forecast (Next Month)', value: data.monthlyForecast, sub: data.model === 'prophet' ? `±${Math.round((data.monthlyForecastUpper - data.monthlyForecastLower) / 2)} range • ${data.monthlyRevenue}` : `${data.monthlyRevenue}`, icon: '📆', gradient: 'linear-gradient(135deg, #111, #111)' },
          ...
          { label: 'Trend Direction', value: data.trend === 'up' ? '📈 Rising' : data.trend === 'down' ? '📉 Declining' : '➡️ Stable', sub: `Model: ${data.model === 'prophet' ? 'Prophet (AI)' : 'Moving Average'}`, icon: '🎯', gradient: data.trend === 'up' ? 'linear-gradient(135deg, #111, #555)' : data.trend === 'down' ? 'linear-gradient(135deg, #ba1a1a, #e07c00)' : 'linear-gradient(135deg, #666, #888)' },
        ].map(c => (
