...
export default function HeatMap() {
  ...
  useEffect(() => {
    ...
    data.heatPoints.forEach((p, i) => {
      ...
      ctx.fillStyle = p.count > 5 ? '#d32f2f' : p.count > 2 ? '#e07c00' : '#555';
      ctx.fill();
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 2;
      ctx.stroke();
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 28, marginBottom: 28 }}>
        <div ref={containerRef} style={{ background: '#fff', borderRadius: 16, border: '1px solid #eaeaea', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', padding: 12, height: 500, position: 'relative' }}>
          <canvas ref={canvasRef} style={{ width: '100%', height: '100%', borderRadius: 8 }} />
          <div style={{ position: 'absolute', bottom: 16, right: 16, display: 'flex', gap: 8, alignItems: 'center', background: 'rgba(255,255,255,0.9)', padding: '4px 12px', borderRadius: 8 }}>
            <span style={{ width: 12, height: 12, borderRadius: '50%', background: '#555' }} />
            <span style={{ fontSize: 11, color: '#666' }}>Low</span>
            <span style={{ width: 12, height: 12, borderRadius: '50%', background: '#e07c00' }} />
            <span style={{ fontSize: 11, color: '#666' }}>Medium</span>
            <span style={{ width: 12, height: 12, borderRadius: '50%', background: '#d32f2f' }} />
            <span style={{ fontSize: 11, color: '#666' }}>High</span>
          </div>
        </div>
            ...
            {sortedPoints.map((p, i) => (
              <div key={i}
                ...
                style={{
                  ...
                  background: selectedWard === p.ward ? '#f5f5f5' : 'transparent',
                  transition: 'background 0.2s',
                }}
              >
                <span style={{
                  width: 10, height: 10, borderRadius: '50%', flexShrink: 0,
                  background: p.count > 5 ? '#d32f2f' : p.count > 2 ? '#e07c00' : '#555',
                }} />
                <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: '#1a1a1a' }}>{p.ward}</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: '#111' }}>{p.count}</span>
                <div style={{ width: 40, height: 4, background: '#f5f5f5', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ width: `${(p.count / maxHeat) * 100}%`, height: '100%', background: p.count > 5 ? '#d32f2f' : p.count > 2 ? '#e07c00' : '#555', borderRadius: 2 }} />
                </div>
              </div>
            ))}
          </div>
