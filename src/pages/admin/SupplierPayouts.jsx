...
export default function SupplierPayouts() {
  ...
  useEffect(() => {
    if (!localStorage.getItem('token') || profile.role !== 'admin') {
      navigate('/auth?tab=login', { replace: true });
      return;
    }
    fetchPayouts();
  }, [days, navigate, profile.role]);
  ...
  const navItems = [
    { label: 'Dashboard', icon: '🏠', nav: '/admin/dashboard' },
  ...
  return (
    <DashboardShell
      ...
      navItems={navItems}
      profile={`Admin: ${user.username || 'Admin'}`}
    >
      <div style={{ padding: '24px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#111' }}>Supplier Payouts</h1>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#fafafa', borderBottom: '1px solid #eee' }}>
                    ...
                    <th style={{ padding: '14px 16px', textAlign: 'center', fontWeight: 600, fontSize: 12, color: '#666', textTransform: 'uppercase' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {suppliers.map((s) => (
